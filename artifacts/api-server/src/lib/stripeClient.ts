...
async function getStripeCredentials(): Promise<StripeCredentials> {
  ...
  // This connector exposes keys as `secret` / `publishable`; older variants
  // used `secret_key` / `publishable_key`. Accept both.
 ...
 * Not cached -- fetched fresh so rotated keys are picked up.
 */
export async function getStripePublishableKey(): Promise<string> {
  const { publishableKey } = await getStripeCredentials();
  if (!publishableKey) {
    throw new Error("Stripe connection did not provide a publishable key.");
  }
